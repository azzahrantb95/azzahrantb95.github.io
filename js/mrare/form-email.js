...
const mrFormEmail = ($ => {
  ...
  class FormEmail {
    constructor(element) {
      this.form = element;
      this.action =
        this.form.getAttribute(Attribute.ACTION) || Default.FORM_ACTION;
    ...
    initSubmitButton() {
      ...
      this.loadingText =
        this.submitButton.getAttribute(Options.LOADING_TEXT) ||
    ...
    processResponse(response) {
      ...
      const successRedirect = this.form.getAttribute(
        Attribute.SUCCESS_REDIRECT
      );
      ...
      if (response.errorDetail) {
        ...
        console.error(
          response.errorName || Default.ERROR_TEXT,
    ...
    toggleFormLoading(loading) {
      ...
      FormEmail.toggleDisabled(
        this.form.querySelectorAll(Selector.ALL_INPUTS),
        loading
      );
    }
    ...
    toggleSubmitButtonAnimation(animate) {
      ...
      this.submitButton.classList[animate ? "add" : "remove"](
        ClassName.LOADING
      );
    }
    ...
    toggleSubmitButtonText(loading) {
      ...
      this.submitButtonSpan.textContent = loading
        ? this.loadingText
    ...
    static toggleDisabled(elements, disabled) {
      // If loading, set text to loading text, else return to original text.
      mrUtil.forEach(elements, (index, element) =>
